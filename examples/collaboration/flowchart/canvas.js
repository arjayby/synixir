import { createFlowchartModel, canvasSize, colors } from "./model.js";
import { connectionPath } from "./geometry.js";
import { createMotion } from "../whiteboard/motion.js";

export function createFlowchart(room) {
  const kindLabel = kind => ({ process: "Process", decision: "Decision", terminal: "Start / End" })[kind];
  ...
  let edgeFrame;
  const motion = createMotion({ onPaint(node, point) {
  ...
  const edgeLayer = root.querySelector(".edges");
  const targetInput = root.querySelector("#connection-target");
  const branchInput = root.querySelector("#connection-label");
  const edgeLabel = root.querySelector("#edge-label");
  const connectionList = root.querySelector("#connection-list");
  ...
  on(root.querySelector("#add-connection"), "click", () => completeConnection(targetInput.value));
  on(root.querySelector("#pick-target"), "click", () => {
    if (readOnly) return;
  ...
  on(root.querySelector("#delete-connection"), "click", () => { if (!readOnly && selectedEdgeId) model.disconnect(selectedEdgeId); });
  ...
  function syncConnections() {
    ...
    const title = id => items.find(item => item.id === id)?.text || "Untitled";
    ...
    root.querySelector("#connections-empty").hidden = edges.length > 0;
    scheduleEdges();
  }
  ...
  function renderInspector() {
    ...
    root.querySelector("#selection-empty").hidden = Boolean(item || selectedEdgeId);
    root.querySelector("#edge-fields").hidden = !selectedEdgeId;
    ...
    root.querySelector("#delete-connection").disabled = readOnly;
    root.querySelector("#pick-target").textContent = connecting ? "Cancel connection" : "Pick on canvas";
    root.querySelector("#pick-target").setAttribute("aria-pressed", String(connecting));
    viewport.classList.toggle("is-connecting", connecting);
    root.querySelector("#canvas-help").textContent = connecting ? "Choose the next node · Escape to cancel" : "Scroll to explore · Select a node to edit or connect";
    if (!item) return;
    ...
    for (const input of [targetInput, branchInput, root.querySelector("#add-connection"), root.querySelector("#pick-target")]) input.disabled = readOnly || !targetInput.options.length;
    root.querySelector("#selection-title").textContent = kindLabel(item.kind);
