...
import { createTableModel, cellKey, limits } from "./model.ts";
...
export function createCollaborativeTable(room: SynixirRoom) {
  ...
  const make = <K extends keyof HTMLElementTagNameMap>(tag: K, className = "", text?: string) => {
    const node = document.createElement(tag); node.className = className;
    if (text !== undefined) node.textContent = text;
    return node;
  };
  let readOnly = true, disposed = false, frame: number|undefined;
  let selection: { [x: string]: any; row: any; column: any; }|null = null, editing: { view: any; mount: any; permission: any; }|null = null, presenceActive = false;
  let rows: import("./model.ts").AxisItem[] = [], columns: import("./model.ts").AxisItem[] = [];
  ...
  const announce = (message: string|null) => { root.querySelector<HTMLElement>("#table-announcement")!.textContent = message; };
  root.innerHTML = `
  ...
  const table = root.querySelector<HTMLElement>("table")!, head = table.querySelector<HTMLElement>("thead tr")!, body = table.querySelector<HTMLElement>("tbody")!;
  const nameInput = root.querySelector<HTMLInputElement>("#column-name")!;
  const addRow = root.querySelector<HTMLButtonElement>("#add-row")!, appendRow = root.querySelector<HTMLButtonElement>("#append-row")!, addColumn = root.querySelector<HTMLButtonElement>("#add-column")!;
  const currentCell = () => selection && cells.get(cellKey(selection!.row, selection!.column));
  const address = (row: string, column: string) => `${String.fromCharCode(65 + columns.findIndex(item => item.id === column))}${rows.findIndex(item => item.id === row) + 1}`;
  function publish() {
    if (disposed) return;
  ...
  function choose(row: any, column: any, focus = true) {
  ...
  function navigate(dr: number, dc: number, wrap = false) {
  ...
  function beginEditing(initial?: string) {
    ...
    const view = new EditorView({ parent: mount, doc: text.toString(), extensions: [
      ...
      keymap.of([...yUndoManagerKeymap.map(binding => ({ ...binding, run: (editor: EditorView) => readOnly || (binding.run?.(editor) ?? false) })), ...defaultKeymap]),
  ...
  on(table, "focusin", (event) => {
    if ((event.target as Element).matches("[data-cell]")) choose((event.target as HTMLElement).dataset.row, (event.target as HTMLElement).dataset.column, false);
  });
  on(table, "click", (event) => {
    if ((event.target as Element).closest<HTMLElement>(".cm-editor")) return;
  ...
  on(table, "dblclick", (event) => { if ((event.target as Element).closest<HTMLElement>("[data-cell]")) beginEditing(); });
  on(table, "keydown", (event) => {
    if ((event.target as Element).closest<HTMLElement>(".cm-editor") || !selection || event.isComposing) return;
    const delta = { ArrowLeft: [0, -1], ArrowRight: [0, 1], ArrowUp: [-1, 0], ArrowDown: [1, 0] }[event.key];
    if (delta) { event.preventDefault(); navigate(delta[0], delta[1]); }
  ...
  on(root.querySelector<HTMLButtonElement>("#edit-cell")!, "click", () => beginEditing());
  on(root.querySelector<HTMLButtonElement>("#clear-cell")!, "click", () => { if (!readOnly && selection) { finishEditing(); model.clear(selection!.row, selection!.column); } });
  for (const axis of ["row", "column"]) on(root.querySelector<HTMLButtonElement>(`#delete-${axis}`)!, "click", () => {
  ...
  on(root, "keydown", (event) => {
    if ((event.target as Element).closest<HTMLElement>(".cm-editor") || event.isComposing || !(event.ctrlKey || event.metaKey) || event.altKey) return;
  ...
  on(root, "focusout", (event) => {
    if (!root.contains(event.relatedTarget as Node | null)) { presenceActive = false; room.awareness.setLocalStateField("cursor", null); publish(); }
    if (editing && !editing.mount.contains(event.relatedTarget as Node | null)) {
  ...
  function renderSelection() {
    ...
    root.querySelector<HTMLElement>("#cell-address")!.textContent = selection ? address(selection!.row, selection!.column) : "Select a cell";
    root.querySelector<HTMLElement>("#table-selection-empty")!.hidden = Boolean(column);
    root.querySelector<HTMLElement>("#table-selection-fields")!.hidden = !column;
    if (column && nameInput.value !== column.label) nameInput.value = column.label ?? "";
    nameInput.readOnly = readOnly;
    for (const id of ["clear-cell", "delete-row", "delete-column"]) root.querySelector<HTMLButtonElement>(`#${id}`)!.disabled = readOnly;
    root.querySelector<HTMLButtonElement>("#edit-cell")!.textContent = readOnly ? "View cell" : "Edit cell";
  }
  function renderPresence() {
    ...
    for (const [key, cell] of cells) {
      ...
      const title = current.map((peer: { name: any; action: any; }) => `${peer.name} ${peer.action}`).join(" · ");
  ...
  function order(parent: HTMLElement, children: any[]) { children.forEach((child: any, index: number) => { if (parent.children[index] !== child) parent.insertBefore(child, parent.children[index] ?? null); }); }
  function render() {
    if (disposed) return;
    ...
    if (selection && (!rows.some(row => row.id === selection!.row) || !columns.some(column => column.id === selection!.column))) {
      ...
      root.querySelector<HTMLElement>(".table-viewport")!.focus(); announce("The selected row or column was removed.");
    }
    ...
    root.querySelector<HTMLElement>("#table-count")!.textContent = `${rows.length} rows · ${columns.length} columns`;
    root.querySelector<HTMLElement>("#table-empty")!.hidden = Boolean(rows.length && columns.length);
  ...
  render();
  function destroy() {
    disposed = true; if (frame !== undefined) cancelAnimationFrame(frame); events.abort();
