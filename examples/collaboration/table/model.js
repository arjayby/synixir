...
export const cellKey = (row, column) => JSON.stringify([row, column]);

export function createTableModel(doc) {
  const rows = doc.getMap("collaborative-table:rows:v1");
  const columns = doc.getMap("collaborative-table:columns:v1");
  const deleted = doc.getMap("collaborative-table:deleted:v1");
  const labels = doc.getMap("collaborative-table:labels:v1");
  const origin = {};
  const history = new Y.UndoManager([rows, columns, deleted, labels], { trackedOrigins: new Set([origin]) });
  const texts = new Map();
  function list(axis) {
    const defaults = axis === "rows" ? initialRows : initialColumns;
  ...
  return {
    history, list, text,
    add(axis) { if (!["rows", "columns"].includes(axis)) return null; return action(() => append(axis)); },
    rename(id, value) {
    ...
    remove(axis, id) {
      if (!["rows", "columns"].includes(axis) || !list(axis).some(item => item.id === id)) return;
