...
import { createWhiteboardModel, canvasSize, colors } from "./model.ts";
...
export function createWhiteboard(room: SynixirRoom) {
  ...
  function renderInspector() {
    const item = selected();
    root.querySelector<HTMLElement>("#selection-empty")!.hidden = Boolean(item);
    root.querySelector<HTMLElement>("#selection-fields")!.hidden = !item;
    if (!item) return;
    root.querySelector<HTMLElement>("#selection-title")!.textContent = item.kind === "sticky" ? "Sticky note" : item.kind === "ellipse" ? "Ellipse" : "Rectangle";
