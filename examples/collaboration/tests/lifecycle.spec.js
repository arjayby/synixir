...
async function expectDocument(page, expected) {
  ...
  await expect.poll(() => page.evaluate(async () => {
    ...
    const text = EditorView.findFromDOM(document.querySelector(".cm-editor")).state.doc.toString();
    const digest = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(text));
    return Array.from(new Uint8Array(digest), byte => byte.toString(16).padStart(2, "0")).join("");
  })).toBe(digest);
}

async function edit(page, changes) {
  await page.evaluate(async changes => {
    const { EditorView } = await import("/node_modules/.vite/deps/@codemirror_view.js");
    EditorView.findFromDOM(document.querySelector(".cm-editor")).dispatch({ changes });
  }, changes);
}
