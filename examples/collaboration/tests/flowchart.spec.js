...
test("flowchart arrows stay attached on every interpolated drag frame and reverse paths are distinct", async ({ page, context, baseURL }) => {
  ...
  await peer.evaluate(() => {
    const shape = document.querySelector('.flow-node');
    const arrow = document.querySelector('.flow-edge');
  ...
  expect(frames.filter(frame => frame.y > 1 && frame.y < 59).length).toBeGreaterThanOrEqual(2);
  expect(Math.max(...frames.map(frame => frame.separation))).toBeLessThan(1);
  await page.mouse.up(); await saved(page);
  await connect(page, "Start / End: Done", "Draft", "Retry");
  await expect(peer.locator('.flow-edge')).toHaveCount(2);
  const curves = await peer.locator('.flow-edge').evaluateAll(paths => paths.map(path => {
