...
const card = (page, title) => page.getByRole("button", { name: `Open card: ${title}`, exact: true });
...
test("viewers can inspect cards but cannot edit, and revocation freezes an open editor", async ({ page, browser, baseURL }) => {
  ...
  try {
    ...
    const setRole = async role => expect((await api(page.request, baseURL,
...
test("board navigation stays on Kanban and the layout fits mobile", async ({ page, baseURL }) => {
  ...
  await expect(page.getByRole("link", { name: "Try the text editor" })).toHaveAttribute("href", `${baseURL}/?room=${roomId}`);
