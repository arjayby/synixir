...
const cell = (page, address) => page.getByRole("gridcell", { name: new RegExp(`^${address}:`) });
const display = (page, address) => cell(page, address).locator(".cell-display");
...
async function paste(page, address, value) {
  await cell(page, address).click();
  await cell(page, address).evaluate((node, value) => {
...
test("table shares same-cell typing, remote carets and selections while preserving the focused editor", async ({ page, context, baseURL }) => {
  ...
  await peer.evaluate(() => { window.originalCellEditor = document.querySelector('.cm-content'); });
  ...
  expect(await peer.evaluate(() => window.originalCellEditor === document.querySelector('.cm-content'))).toBe(true);
...
test("table merges disconnected edits to one cell and restores data and schema after restart", async ({ page, context, baseURL, backend }) => {
  ...
  await fresh.getByRole("link", { name: "Try the multiplayer form", exact: true }).click();
...
test("table viewers cannot type, paste, delete or undo; live downgrades and mobile work", async ({ page, browser, baseURL }) => {
  ...
  try {
    ...
    const role = async value => expect((await api(page.request, baseURL, `/api/rooms/${roomId}/members/${user.username}`, "PUT", { role: value })).ok()).toBe(true);
