...
export const test = base.extend<{ pageErrors: void }, { backend: Backend }>({
  pageErrors: [async ({ context }, use) => {
    const errors: string[] = [];
    const listen = (page: Page) => page.on("pageerror", error => errors.push(error.message));
  ...
  backend: [async ({}, use) => {
    ...
    async function start() {
      ...
      for (const stream of [child.stdout, child.stderr]) {
        stream!.on("data", (chunk: string) => { output = (output + chunk).slice(-12000); });
      }
    ...
    try {
      ...
      await use({
        ...
        compact: (room) => {
          ...
          const maintenanceEnv: NodeJS.ProcessEnv = { ...env, SYNIXIR_TEST_ROOM: room };
