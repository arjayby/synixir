...
const document = page => page.getByRole("textbox", { name: "Rich text document", exact: true });
const textWithoutCursors = locator => locator.evaluate(node => {
  const copy = node.cloneNode(true);
  copy.querySelectorAll(".rich-caret").forEach(caret => caret.remove());
  return copy.textContent;
});
const content = (locator, text) => expect.poll(() => textWithoutCursors(locator)).toBe(text);
...
test("rich text supports lists and quotes and keeps plain text content and cursors separate", async ({ page, context, baseURL }) => {
  ...
  await expect(page.getByRole("link", { name: "Try the text editor", exact: true })).toHaveAttribute("href", `${baseURL}/?room=${roomId}`);
});
