...
async function open(ctx, roomId, role) {
...
async function rejectsOrigin(origin) {
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on("upgrade", (_response, socket) => { socket.destroy(); reject(new Error("Untrusted origin upgraded")); });
    req.on("error", reject);
...
try {
  if (mode === "seed") {
    ...
    for (const [person, role] of [[editor, "editor"], [viewer, "viewer"]]) {
    ...
    const manifest = { roomId, content: content + " offline", title: "Packaged SDK settings", accounts: {} };
    for (const [role, person] of [["owner", owner], ["editor", editor], ["viewer", viewer]]) {
