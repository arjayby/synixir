...
const clients = [];
const groups = Array.from({ length: roomCount }, () => []);
const saveMs = [], convergeMs = [], connectMs = [];
let updateBytes = 0;
...
function account() {
  ...
  let csrf;
  return async (path, method = "GET", body, signal = AbortSignal.timeout(10000)) => {
...
async function metrics() {
  ...
  const value = name => Number(text.match(new RegExp(`^${name} (.+)$`, "m"))?.[1] ?? 0);
...
async function join(api, user, roomId, index) {
...
try {
  ...
  for (const { room } of clients) room.doc.on("update", (update, source) => {
