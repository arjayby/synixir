...
try {
  ...
  assert.ok(pack.files.some(file => file.path === "index.d.ts"));
  assert.ok(pack.files.some(file => file.path === "README.md"));
  assert.ok(pack.files.every(file => /^(src\/|index\.d\.ts$|README\.md$|package\.json$)/.test(file.path)));
  ...
  writeFileSync(join(temporary, "package.json"), JSON.stringify({ private: true, type: "module", dependencies: {
    ...
    typescript: workspace.devDependencies.typescript, vite: example.devDependencies.vite,
  } }));
