...
import { SynixirRoom, SynixirError, type RoomState, type RoomAccess, type AccessRequest, type RoomOptions } from "@synixir/client";
...
test("validates options before allocating a document or awareness timer", () => {
  for (const roomId of ["", "bad/id", "_invalid", "x".repeat(129), 123]) {
    assert.throws(() => new SynixirRoom({ ...options, roomId } as RoomOptions), TypeError);
  }
  ...
  assert.throws(() => new SynixirRoom({ ...options, getAccess: true as unknown as RoomOptions["getAccess"] }), TypeError);
});
...
test("coalesces connects, aborts pending access, and ignores a late old result", async () => {
  const requests: (AccessRequest & { resolve: (access: RoomAccess) => void })[] = [];
...
test("nonretryable access errors permit explicit retry and reject malformed grants", async () => {
  ...
  const room = new SynixirRoom({ ...options, getAccess: async () => {
    ...
    return { token: "grant", userId: options.userId, role: "admin" as RoomAccess["role"] };
  } });
  try {
...
test("destroy cancels scheduled recovery and subscriptions stop after final state", async () => {
  ...
  const states: RoomState["connection"][] = [];
