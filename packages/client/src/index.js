...
export class SynixirRoom {
  ...
  #getAccess;
  #listeners = new Set();
  #state = Object.freeze({ connection: "disconnected", role: null, readOnly: true,
  ...
  constructor({ roomId, userId, serverUrl = globalThis.location?.origin, getAccess } = {}) {
  ...
  #publish(changes) {
    ...
    if (Object.keys(state).every(key => state[key] === this.#state[key])) return;
  ...
  async #start(generation, signal) {
    ...
    try {
      ...
      const provider = new PhoenixChannelProvider(transport, `document:${this.#roomId}`, this.#doc, {
  ...
  #stop(error) {
    ...
    if (active) {
      ...
      this.#socketClosed = this.#socketClosed.then(() => new Promise(resolve => active.socket.disconnect(resolve)));
    }
